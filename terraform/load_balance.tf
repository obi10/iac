

resource "oci_load_balancer" "lb_tf01" {
    compartment_id      = "${var.compartment_ocid}"
    display_name        = "LB_TF01"
    shape               = "400Mbps"
    subnet_ids = ["${oci_core_subnet.subnet2.id}",
                  "${oci_core_subnet.subnet3.id}"]
}

resource "oci_load_balancer_backend_set" "lb-backset" {
  name             = "lb-backset"
  load_balancer_id = "${oci_load_balancer.lb_tf01.id}"
  policy           = "ROUND_ROBIN"

  health_checker {
    port                = "80"
    protocol            = "HTTP"
    response_body_regex = ".*"
    url_path            = "/"
  }
   provisioner "local-exec" {
    command = "sleep 5"
  }

}

resource "oci_load_balancer_backend" "lb-be1" {
  load_balancer_id = "${oci_load_balancer.lb_tf01.id}"
  backendset_name  = "${oci_load_balancer_backend_set.lb-backset.name}"
  ip_address       = "${oci_core_instance.instance1.private_ip}"
  port             = 80
  backup           = false
  drain            = false
  offline          = false
  weight           = 1
}

resource "oci_load_balancer_backend" "lb-be2" {
  load_balancer_id = "${oci_load_balancer.lb_tf01.id}"
  backendset_name  = "${oci_load_balancer_backend_set.lb-backset.name}"
  ip_address       = "${oci_core_instance.instance2.private_ip}"
  port             = 80
  backup           = false
  drain            = false
  offline          = false
  weight           = 1
}

/*resource "oci_load_balancer_hostname" "host01" {
  #Required
  hostname         = "${oci_core_instance.instance1.private_ip}" 
  load_balancer_id = "${oci_load_balancer.lb_tf01.id}"
  name             = "host01"
}

resource "oci_load_balancer_hostname" "host02" {
  #Required
  hostname         = "${oci_core_instance.instance2.private_ip}"
  load_balancer_id = "${oci_load_balancer.lb_tf01.id}"
  name             = "host02"
}
*/
#resource "oci_load_balancer_rule_set" "test_rule_set" {
#  items {
#    action = "ADD_HTTP_REQUEST_HEADER"
#    header = "example_header_name"
#    value  = "example_header_value"
#  }
#
#  load_balancer_id = "${oci_load_balancer.lb_tf01.id}"
#  name             = "example_rule_set_name"
#}

resource "oci_load_balancer_listener" "lb-listener1" {
  load_balancer_id         = "${oci_load_balancer.lb_tf01.id}"
  name                     = "http"
  default_backend_set_name = "${oci_load_balancer_backend_set.lb-backset.name}"
  #hostname_names           = ["${oci_load_balancer_hostname.host01.name}", "${oci_load_balancer_hostname.host02.name}"]
  port                     = 80
  protocol                 = "HTTP"
  #rule_set_names           = ["${oci_load_balancer_rule_set.test_rule_set.name}"]

  connection_configuration {
    idle_timeout_in_seconds = "2"
  }
}

/*resource "oci_load_balancer_certificate" "lb-cert1" {
  load_balancer_id   = "${oci_load_balancer.lb_tf01.id}"
  ca_certificate     = "-----BEGIN CERTIFICATE-----\\nj1/IHmj84LNGp7pHCyiYx/oI+00gRch04H2pJv0TP3sAQ37gplBwDrUo\n-----END CERTIFICATE-----"
  certificate_name   = "certificate1"
  private_key        = "-----BEGIN RSA PRIVATE KEY-----\\nvwqbG6b0NEYLRPLEdsS/c8TV9mMlvb0EW+GXfmkpTrTNt3hyXniu\n-----END RSA PRIVATE KEY-----"
  public_certificate = "-----BEGIN CERTIFICATE-----\\nj1/IHmj84LNGp7pHCyiYx/oI+00gRch04H2pJv0TP3sAQ37gplBwDrUo\n-----END CERTIFICATE-----"

  lifecycle {
    create_before_destroy = true
  }
}*/

output "Load Balance" {
  value = ["${oci_load_balancer.lb_tf01.ip_addresses}"]
}

#--------------------------------------------------------------------------
# Load Balancer 
/*
resource "oci_load_balancer_certificate" "lb-cert1" {
  load_balancer_id   = "${oci_load_balancer.lb_tf01.id}"
  ca_certificate     = "-----BEGIN CERTIFICATE-----\\nj1/IHmj84LNGp7pHCyiYx/oI+00gRch04H2pJv0TP3sAQ37gplBwDrUo\n-----END CERTIFICATE-----"
  certificate_name   = "certificate1"
  private_key        = "-----BEGIN RSA PRIVATE KEY-----\\nvwqbG6b0NEYLRPLEdsS/c8TV9mMlvb0EW+GXfmkpTrTNt3hyXniu\n-----END RSA PRIVATE KEY-----"
  public_certificate = "-----BEGIN CERTIFICATE-----\\nj1/IHmj84LNGp7pHCyiYx/oI+00gRch04H2pJv0TP3sAQ37gplBwDrUo\n-----END CERTIFICATE-----"

  lifecycle {
    create_before_destroy = true
  }
}

resource "oci_load_balancer_path_route_set" "test_path_route_set" {
  #Required
  load_balancer_id = "${oci_load_balancer.lb_tf01.id}"
  name             = "pr-set1"

  path_routes {
    #Required
    backend_set_name = "${oci_load_balancer_backend_set.lb-backset.name}"
    path             = "/example/video/123"

    path_match_type {
      #Required
      match_type = "EXACT_MATCH"
    }
  }
}





resource "oci_load_balancer_listener" "lb-listener1" {
  load_balancer_id         = "${oci_load_balancer.lb_tf01.id}"
  name                     = "http"
  default_backend_set_name = "${oci_load_balancer_backend_set.lb-bes1.name}"
  hostname_names           = ["${oci_load_balancer_hostname.test_hostname1.name}", "${oci_load_balancer_hostname.test_hostname2.name}"]
  port                     = 80
  protocol                 = "HTTP"
  rule_set_names           = ["${oci_load_balancer_rule_set.test_rule_set.name}"]

  connection_configuration {
    idle_timeout_in_seconds = "2"
  }
}

resource "oci_load_balancer_listener" "lb-listener2" {
  load_balancer_id         = "${oci_load_balancer.lb1.id}"
  name                     = "https"
  default_backend_set_name = "${oci_load_balancer_backend_set.lb-bes1.name}"
  port                     = 443
  protocol                 = "HTTP"

  ssl_configuration {
    certificate_name        = "${oci_load_balancer_certificate.lb-cert1.certificate_name}"
    verify_peer_certificate = false
  }
}

resource "oci_load_balancer_backend" "lb-be1" {
  load_balancer_id = "${oci_load_balancer.lb1.id}"
  backendset_name  = "${oci_load_balancer_backend_set.lb-bes1.name}"
  ip_address       = "${oci_core_instance.instance1.private_ip}"
  port             = 80
  backup           = false
  drain            = false
  offline          = false
  weight           = 1
}

resource "oci_load_balancer_backend" "lb-be2" {
  load_balancer_id = "${oci_load_balancer.lb1.id}"
  backendset_name  = "${oci_load_balancer_backend_set.lb-bes1.name}"
  ip_address       = "${oci_core_instance.instance2.private_ip}"
  port             = 80
  backup           = false
  drain            = false
  offline          = false
  weight           = 1
}

resource "oci_load_balancer_rule_set" "test_rule_set" {
  items {
    action = "ADD_HTTP_REQUEST_HEADER"
    header = "example_header_name"
    value  = "example_header_value"
  }

  load_balancer_id = "${oci_load_balancer.lb1.id}"
  name             = "example_rule_set_name"
}

output "lb_public_ip" {
  value = ["${oci_load_balancer.lb1.ip_addresses}"]
}
*/